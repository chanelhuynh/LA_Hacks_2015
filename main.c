...
static Window *s_main_window_t;   // Notes + Count down
...
static void main_window_load_n(Window *window) {
  ...
  //FONT_MM_24 = fonts_load_custom_font(resource_get_handle(FONT_MM_24));
  //text_layer_set_font(s_font_layer, FONT_MM_24);
...
static void tick_handler(struct tm* tick_time, TimeUnits units_changed) {
  ...
  if (min == 0 && sec == 0) {
    ...
    light_enable(true);
    vibes_long_pulse();
    psleep(2000);      // pebble guy: use apptimer instead if it screws up other stuff
...
static void main_window_load_t(Window *window) {
  //Layer *note_layer = window_get_root_layer(window);
  
  Layer *window_layer = window_get_root_layer(window);
  GRect window_bounds = layer_get_bounds(window_layer);
  
  //s_output_layer_t = text_layer_create(GRect(5, 0, window_bounds.size.w - 10 ,window_bounds_size.h/2));
